...
class DataLoader3D(SlimDataLoaderBase):
    ...
    def generate_train_batch(self):
        ...
        for j, i in enumerate(selected_keys):
            ...
            ## for debugging
            # with open("/root/autodl-tmp/3DLoader_case_all_data.pkl", "wb") as f:
            #     pickle.dump(case_all_data, f)
            # pdb.set_trace()
        ...
        ## For debugging
        # with open("/root/autodl-tmp/3DLoader_seg.pkl", "wb") as f:
        #     pickle.dump(seg, f)
        # pdb.set_trace()

        return {'data': data, 'seg': seg, 'properties': case_properties, 'keys': selected_keys}

class DataLoader3DdistMap(DataLoader3D):
    def __init__(self, data, patch_size, final_patch_size, batch_size, has_prev_stage=False,
                 oversample_foreground_percent=0.0, memmap_mode="r", pad_mode="edge", pad_kwargs_data=None,
                 pad_sides=None):
        """
        ...
        :param oversample_foreground: half the batch will be forced to contain at least some foreground (equal prob for each of the foreground classes)
        """
        super(DataLoader3DdistMap, self).__init__(data, patch_size, final_patch_size, batch_size, has_prev_stage,
    ...
    def generate_train_batch(self):
        ...
        for j, i in enumerate(selected_keys):
            ...
            # load origin dist map
            # if isfile(self._data[i]['data_file'][:-4] + ".dist_map.npy"):
            #     origin_dist_map = np.load(self._data[i]['data_file'][:-4] + ".dist_map.npy", self.memmap_mode)

            # for debugging
            # with open("/root/autodl-tmp/3DLoader_case_all_data_distMap.pkl", "wb") as f:
            #     pickle.dump(case_all_data, f)
            # pdb.set_trace()
            ...
            # [data, dist_map, label]
            data[j] = np.pad(case_all_data[[0]], ((0, 0),
                                                  (-min(0, bbox_x_lb), max(bbox_x_ub - shape[0], 0)),
                                                  (-min(0, bbox_y_lb), max(bbox_y_ub - shape[1], 0)),
                                                  (-min(0, bbox_z_lb), max(bbox_z_ub - shape[2], 0))),
                             self.pad_mode, **self.pad_kwargs_data)
            ...
            seg[j, 0] = np.pad(case_all_data[[2]], ((0, 0),
                                                    (-min(0, bbox_x_lb), max(bbox_x_ub - shape[0], 0)),
                                                    (-min(0, bbox_y_lb), max(bbox_y_ub - shape[1], 0)),
                                                    (-min(0, bbox_z_lb), max(bbox_z_ub - shape[2], 0))),
                               'constant', **{'constant_values': -1})
